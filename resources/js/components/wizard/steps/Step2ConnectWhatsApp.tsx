...
export function Step2ConnectWhatsApp({
    ...
    // Show fallback button if QR doesn't arrive within timeout
    useEffect(() => {
        if (isWaitingForQr && !qrCode) {
            const timer = setTimeout(() => setShowFallback(true), QR_TIMEOUT);
